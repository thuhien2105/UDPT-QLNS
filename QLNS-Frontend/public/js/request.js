        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                if (data && data.request) {
                    ...
                    const requestType = data.request.request_type;
                    const selectElement = $("#category_id_0");
                    ...
                    options.forEach((option) => {
                        const isSelected =
                            option.value === requestType ? "selected" : "";
                        selectElement.append(
                            `<option value="${option.value}" ${isSelected}>${option.text}</option>`
                        );
                    });

                    // Set other form fields
                    $("#date_start_0").val(
                        data.request.start_time.split("T")[0]
                    ); // Chỉ lấy phần ngày
                    $("#date_end_0").val(data.request.end_time.split("T")[0]); // Chỉ lấy phần ngày
                    $("#reason").val(data.request.reason); // Sử dụng .val() cho input
                    $("#request_owner_id_0").val(data.request.employee_id);
